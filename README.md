...
9. [Git filterbranch 過濾數據庫中的提交歷史](#GitFilterbranch過濾git數據庫中的提交歷史)       
    [❗移除數據庫中的敏感信息或文件](#移除數據庫中的敏感信息或不需要的文件)      
...
12. [情境題](#情境題)       
    [❗暫存進度](#暫存進度)    
    [❗不小心把檔案刪掉了](#不小心把檔案刪掉了)        
    [❗斷頭狀態+基於特定提交進行試驗](#斷頭狀態基於特定提交進行試驗和測試)       
- [參考資源](#參考資源)        
...
1. [斷頭狀態 特性與用途](#斷頭狀態基於特定提交進行試驗和測試) 
    - 利用斷頭狀態進行 簡單的功能實驗
...
- 重置(reset)      
    ...
    3. 新提交：
    如果你在這個狀態下建立新的提交 a4，a4 將會建立在 a2 的基礎上。 
    a4 會成為分支 A 的一部分，而分支歷史將變成 a1 > a2 > a4。
...
1. 在所有提交歷史中刪除特定檔案 可復原
    ...
    #說明：遍歷每個提交 並在每個提交中執行指定的shell命令，具體來說是刪除每個交中名為".env"的文件。
    # --tree-filter
...
- 檔案尚未被提交：在工作目錄\暫存區被誤刪
    ...
    #說明：在正常模式下，當你執行 git checkout 並指定一個已經存在於當前分支的文件時，
    #      Git 會從最新提交中恢復該文件的內容，並將其放入你的工作目錄中。
    ```
- 檔案已提交被誤刪
    ```bash
    git reset HEAD^
